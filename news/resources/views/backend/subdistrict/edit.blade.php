...
<form class="forms-sample" method="POST" action="{{ route('update.subdistrict',$subdistrict->id) }}">
  ...
  <div class="form-group">
    <label for="exampleInputEmail1">SubDistrict Arabic</label>
    <input type="text" class="form-control" name="subdistrict_fr"  value="{{ $subdistrict->subdistrict_fr }}">
     @error('subdistrict_fr')
    <span class="text-danger">{{ $message }}</span>
    @enderror
  </div>
...
<div class="form-group">
 ...
 <option disabled="" selected="">--Select District</option>
    ...
    <option value="{{ $row->id }}"
      ...
      >{{ $row->district_en }} | {{ $row->district_fr }}</option>
             @endforeach
